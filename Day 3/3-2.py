...
for i in range(len(lines)):
    ...
    for j in range(len(eachline)):
        ...
        if eachchar.isdigit():
            num+=eachchar
            if (j+1 < len(eachline) and is_symbol(eachline[j+1])):
                star_index = str(i)+","+str(j+1)
                isValid = True
            if (j-1 >= 0 and is_symbol(eachline[j-1])):
                star_index = str(i)+","+str(j-1)
                isValid = True
            if (i+1 < len(lines) and is_symbol(lines[i+1][j])):
                star_index = str(i+1)+","+str(j)
                isValid = True
            if (i-1 >= 0 and is_symbol(lines[i-1][j])):     
                star_index = str(i-1)+","+str(j)
                isValid = True
                
            if (i+1 < len(lines) and j-1 >= 0 and is_symbol(lines[i+1][j-1])):
                star_index = str(i+1)+","+str(j-1)
                isValid = True
            if (i+1 < len(lines) and j+1 < len(eachline) and is_symbol(lines[i+1][j+1])):
                star_index = str(i+1)+","+str(j+1)
                isValid = True
            if (i-1 >=0 and j-1 >= 0 and is_symbol(lines[i-1][j-1])):
                star_index = str(i-1)+","+str(j-1)
                isValid = True
            if (i-1 >=0 and j+1 < len(eachline)and is_symbol(lines[i-1][j+1])):
                star_index = str(i-1)+","+str(j+1)
                isValid = True
            ...
            if isComplete:
                if isValid:
                    ...
                    list_of_valid_part_numbers.append([int(num),star_index])
                isValid = False
                isComplete=False
                num = ""
...
for item in list_of_valid_part_numbers:
    key = item[1]  
...
for each in list(result_dict.values()):
    if len(each) <=1:
